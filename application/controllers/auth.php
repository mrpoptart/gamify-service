...
class Auth extends CI_Controller
{
        ...
        if ($this->tank_auth->is_logged_in()) { // logged in
            ...
            if ($this->form_validation->run()) { // validation ok
                ...
                )
                {
                    ...
                    $this->Db_model->update_timezone($_POST['timezone_offset']);
                    redirect('/goals');
                } else {
        ...
        if ($this->tank_auth->is_logged_in()) { // logged in
            ...
            if ($this->form_validation->run()) { // validation ok
                if (!is_null($data = $this->tank_auth->create_user(
                    ...
                    $data['title']="Gamify Registration";
                    ...
                    if ($email_activation) { // send "activate" email
                        ...
                        $this->load->model('Db_model');
                        $this->Db_model->create_goal($data['user_id'], "Create a Gamify Goal", "A treat", date("Y-m-d"));

                        //$this->_show_message($this->lang->line('auth_message_registration_completed_1'));
                        $this->load->view('header_view', $data);
                        $this->load->view('auth/register_complete', $data);
                        $this->load->view('footer_view', $data);
                        return;
     ...
     * User is verified by user_id and Gamify Authentication code in the URL.
