    ...
    <table id="goalsTable" class="table table-border tablesorter">
        <thead>
            <tr>
                ...
                <th>Reward<div class="icon-arrow-down"/></th>
        ...
        <tr>
            ...
            <td><?php echo $goal->reward; ?></td>
            <td><?php
                echo date("Y-m-d",strtotime($goal->due_date));
                //echo relativeTime(strtotime($goal->due_date)+(60*60*24));
                ?></td>
            <td id="form<?php echo $goal->id;?>"><?php if($goal->completed_date != ''):?>
                <?php if($goal->rewarded_date != ''):?>
                    Goal complete!
                <?php else:?>
                        <a onclick="reward(<?php echo $goal->id; ?>)" class="btn btn-success">Claim Reward</a>
                <?php endif;?>
            <?php else: ?>
                <a onclick="done(<?php echo $goal->id; ?>)" class="btn btn-primary">Mark Done</a>
            <?php endif; ?></td>
        </tr>
        <?php endforeach; ?>
    </table>
